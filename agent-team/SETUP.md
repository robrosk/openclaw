...
|-------|------|-----------------|
| **Orchestrator** | Portfolio manager. Triages signals, dispatches work, publishes canonical decisions. | `#portfolio-daily`, `#portfolio-weekly`, `#task-board`, `#watchlist` |
...
Dispatch happens in `#task-board`. No agent executes trades. All handoffs are channel-visible.
...
All secrets and **all** Slack channel IDs are referenced in `openclaw.json` via `${ENV_VAR}` interpolation. Zero channel IDs are hardcoded in the repo — the config ships with env var placeholders only, and the gateway resolves them from `~/.openclaw/.env` on boot. Real values live on the VM at `~/.openclaw/.env`.
...
- **11 Slack channels** created: `#market-signals`, `#research`, `#quant-signals`, `#contrarian`, `#task-board`, `#portfolio-daily`, `#portfolio-weekly`, `#themes`, `#agents`, `#watchlist`, `#weekly-outlook`.
...
- **Scout**: `#market-signals`, `#task-board`, `#watchlist`, `#weekly-outlook`, `#agents`.
- **Analyst**: `#research`, `#market-signals`, `#task-board`, `#watchlist`, `#agents`.
...
- In `#task-board`, ask the Orchestrator to run `explicit_dispatch` against Scout for a manual `build_morning_watchlist` and watch the flow through `#watchlist`.
...
- **Channel-visible handoffs.** All tasking goes through `#task-board`. Follow-up questions go in the specialist's own channel via `follow_up_questions`.
