...
Read `~/.openclaw/skills/channel-directory/channels.json` — it maps `#channel-name` to the real Slack channel ID. Example:

```json
{
  "#task-board": "C0AP1234ABC",
  "#research": "C0AP5678DEF"
}
```

This file is auto-generated by the sync script from `~/.openclaw/.env`. If a channel is missing, the operator has not set the corresponding `SLACK_CHANNEL_*` env var — refuse to post and surface the missing channel name in your reply.
...
```
message(action: "send", target: "<channel id from channels.json>", text: "...")
```

Read:

```
message(action: "read", target: "<channel id from channels.json>")
```
